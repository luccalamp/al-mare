...
export function buildPortalWhatsappMessage(patientName: string, link: string) {
  ...
  return [
    ...
    link,
    "Por la voce acompanha sua pre-consulta, cuidados recomendados (homecare), fotos de evolucao e seus proximos horarios.",
  ].join("\n\n");
}
...
export function buildWhatsappShareUrl(phone: string, message: string) {
  ...
  if (!normalizedPhone) return `https://api.whatsapp.com/send?text=${encodedMessage}`;
...
async function parsePortalRouteResponse(response: Response) {
  ...
  if (!response.ok) {
    const message = payload && typeof payload === "object" && "error" in payload && typeof payload.error === "string" ? payload.error : "Nao foi possivel gerenciar o link do portal agora.";
...
export async function issuePortalLink(clientId: string): Promise<{ token: string; linkActive: boolean }> {
  const response = await fetch("/api/portal/link", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ clientId }) });
...
export async function deactivatePortalLink(clientId: string): Promise<{ linkActive: boolean }> {
  const response = await fetch("/api/portal/link", { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ clientId }) });
  return parsePortalRouteResponse(response);
}
