...
import { Link2, Loader2, Send, ShieldCheck, Globe } from "lucide-react";
...
export default function ClientPreConsultationTab({
  ...
  return (
    <div className="space-y-5 p-4 sm:space-y-6 sm:p-6">
      ...
      <section className="rounded-[32px] border border-[var(--color-brand-line)] bg-[rgba(255,250,243,0.8)] p-5 shadow-[0_18px_45px_rgba(94,58,28,0.08)]">
        <div className="flex items-start gap-3">
          <div className="mt-0.5 rounded-2xl bg-[var(--color-sage-soft)] p-2 text-[var(--color-sage)]">
            <Globe size={18} />
          </div>
          <div className="flex-1 space-y-4">
            <div>
              <p className="text-[11px] font-bold uppercase tracking-[0.28em] text-[var(--color-sage)]">
                Portal do cliente
              </p>
              <h3 className="mt-2 text-lg font-semibold text-[var(--color-text)]">
                Link de acesso pós-atendimento
              </h3>
              <p className="mt-2 text-sm leading-6 text-[var(--color-text-secondary)]">
                Gere um link exclusivo para o cliente acompanhar homecare, fotos de evolução e próximos agendamentos — sem necessidade de login.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-2 sm:flex-row">
              <button
                ...
                disabled={portalBusy}
                className="inline-flex min-h-11 items-center justify-center rounded-xl bg-[var(--color-sage)] px-4 py-3 text-sm font-semibold text-white transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60"
              >
                {portalBusy ? <Loader2 size={16} className="animate-spin" /> : portalLink?.token ? "Gerar novo link" : "Gerar link do portal"}
              </button>
              <button
                type="button"
                onClick={handleSharePortalOnWhatsapp}
                disabled={portalBusy}
                className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-[var(--color-brand-line)] bg-[var(--color-sage-soft)] px-4 py-3 text-sm font-semibold text-[var(--color-sage)] transition hover:bg-white disabled:cursor-not-allowed disabled:opacity-40"
              >
                <Send size={15} /> Enviar no WhatsApp
              </button>
              <button
                ...
                disabled={portalBusy || !portalLink?.token || !portalLink?.linkActive}
                className="inline-flex min-h-11 items-center justify-center rounded-xl border border-[var(--color-brand-line)] bg-white px-4 py-3 text-sm font-semibold text-[var(--color-brand-deep)] transition hover:bg-[var(--color-brand-soft)] disabled:cursor-not-allowed disabled:opacity-40"
              >
                Invalidar link
              </button>
